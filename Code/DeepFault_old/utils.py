...
# LAYERS_NAMES = {
#     'lenet5': ['conv2d_1', 'block1_pool1', 'conv2d_2', 'max_pooling2d_1', 'flatten_1', 'dense_1', 'dense_2']
# }
...
def get_layer_outs(model, test_input):
    ...
    # layer_names = LAYERS_NAMES[model_name]
    layer_names = [layer.name for layer in model.layers[:-1]]
    if 'input_1' in layer_names:
        layer_names.remove('input_1')
        layer_outs.append([np.array([])])
    ...
    ret = temp_model.predict(test_input)
    for layer_output in ret:
        result = np.reshape(layer_output, (num, -1))
        layer_outs.append([result])
    return layer_outs


# modified by Ahmed !!!!!!!!!!!!!!!!!!!!!!!
def get_layer_outs1(model, test_input):
    test_input = test_input.reshape(-1,28,28,1)
    inp = model.input
    layer_outs = []
    for layer in model.layers[1:]:
        layer_model = K.function([model.layers[0].input], [layer.output])
        ret = layer_model(test_input)[0]
        num = test_input.shape[0]
        ret = np.reshape(ret, (num, -1))
        layer_outs.append([ret])
    return layer_outs

# def get_layer_outs_sol1(model, test_input):
#     test_input = test_input.reshape(-1, 28, 28, 1)
#     output_layers = [model.get_layer(layer_name).output for layer_name in ['fc1']]
#     output_layers.append(model.output)
#     temp_model = Model(
#         inputs=model.input,
...
def get_layer_outs_orig_paper(model, test_input):
    inp = model.input  # input placeholder
    outputs = [layer.output for layer in model.layers]  # all layer outputs
...
def construct_spectrum_matrices(model, trainable_layers,
    ...
    for tl in trainable_layers:
        ...
        num_cf.append(np.zeros(layer_outs[tl][0].shape[-1]))  # covered (activated) and failed
        num_uf.append(np.zeros(layer_outs[tl][0].shape[-1]))  # uncovered (not activated) and failed
        num_cs.append(np.zeros(layer_outs[tl][0].shape[-1]))  # covered and succeeded
        num_us.append(np.zeros(layer_outs[tl][0].shape[-1]))  # uncovered and succeeded
        scores.append(np.zeros(layer_outs[tl][0].shape[-1]))


    for tl in trainable_layers:
        layer_idx = trainable_layers.index(tl)
        all_neuron_idx = range(layer_outs[tl][0].shape[-1])
        test_idx = 0
        # a4 = layer_outs[tl]
        # for l in layer_outs[tl][0]:
        for l in layer_outs[tl]:
            for neuron_idx in range(layer_outs[tl][0].shape[-1]):
                # print(l[..., neuron_idx])
                # a1 = np.mean(l[..., neuron_idx])
                # a2 = test_idx in correct_classifications
                # a3 = test_idx in misclassifications
                # a4 = np.mean(l[..., neuron_idx]) > activation_threshold
                # a5 = np.mean(l[..., neuron_idx]) <= activation_threshold
